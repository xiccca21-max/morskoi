...
export function mapApiError(message: unknown, fallback = 'Что-то пошло не так'): string {
  ...
  const table: Record<string, string> = {
    ...
    'SOCKET_TIMEOUT': 'Сервер не ответил вовремя — проверьте интернет и попробуйте снова',
    'SOCKET_DISCONNECTED': 'Нет соединения с сервером — подождите или перезапустите приложение',
